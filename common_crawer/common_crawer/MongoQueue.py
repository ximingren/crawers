...
class MongoQueue():
    ...
    def push(self,id,url,star=None):
        try:
            self.db.insert({'id':id,'star':star,'url':url,'status':self.OUTSTANDING})
            print(id,url,'插入队列成功')
    ...
    def pop(self):
        ...
        datetime.now()
        if record:
            return record['id'],record['url'],record['star']
        else:
            self.repair()
            raise KeyError
    ...
    def completeId(self,id):
        self.db.update({"id": id}, {'$set': {'status': self.COMPLETE}})
...
if __name__ == '__main__':
    ...
    genresList=['剧情','喜剧','动作','爱情','科幻','动画','悬疑','惊悚','恐怖','犯罪','同性','音乐','歌舞','传记','历史','战争','西部','奇幻','冒险','灾难','武侠','情色']
    ...
    # for j in range(780, parts):
    #     for i in maoyan.find().limit(20).skip(j * 20):
            # print(i)
