...
class A58Spider(scrapy.Spider):
    ...
    def parse_pages(self, response):
        ...
        if not response.xpath('.//input[@id="btnSubmit"]'):
            ...
            yield Request(self.url % (area,1), callback=self.parse_price,dont_filter=True, meta={'pages': pages, 'current_page': 1,'area':area})
    ...
    def parse_price(self, response):
        ...
        if not response.xpath('.//input[@id="btnSubmit"]'):
            ...
            for house in house_list:
                ...
                baseinfo = ''.join(
                    list(filter(lambda x: '\n' not in x, house.xpath('./div[@class="list-info"]/p//span/text()').extract())))
