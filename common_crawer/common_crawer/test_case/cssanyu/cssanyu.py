...
if __name__ == '__main__':
    ...
    coderes=session.get(host+code_url[:-3],headers={'Referer':"http://baidupan.xn--wxtn5jlzslkal2y.cssanyu.org/bbs2/forum.php?mod=viewthread&tid=273518&extra=page%3D1"})
    with open('1.png','wb')as f:
        f.write(coderes.content)
    code_value=input()
    veritfyUrl='http://baidupan.xn--wxtn5jlzslkal2y.cssanyu.org/bbs2/misc.php?mod=seccode&action=check&inajax=1&modid=member::logging&idhash=%s&secverify=%s'%(seccode,code_value)
    veritify_station=session.get(veritfyUrl)
    print(veritify_station.text)
