...
def get_list():
    ...
    with PyV8.JSContext() as ctxt:
        with open('rohr.min.js', 'r') as f:
            ...
            print(token_value)
            url = 'http://waimai.meituan.com/ajax/poilist?_token=' + token_value
            res = requests.post(
                url,
...
def get_geoid(addr):
    ...
    res=requests.get('http://waimai.meituan.com/geo/geohash?lat=%s&lng=%s&addr=1'%(lat,lng),headers=headers)
