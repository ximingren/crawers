        ...
        if data['data']['cards']:
            ...
            for k in range(search_size):
                if data['data']['cards'] and data['data']['cards'][1]['card_group'][k]['user']['screen_name'] == name:
                    ...
                    print("----------------%s的订阅者数：%d   粉丝数%d" % (name, subs_size, fans_size))
                    if fans_size > 20:
...
def get_subs(weibo_id, name, headers, subs_list_page):
    ...
    try:
        ...
        for page in range(0, subs_list_page):
            ...
            print("----------------爬取%s的订阅者列表的网址:  %s\n" %(name,subs_url))
            print("----------------请求连接到%s的订阅者列表页面"%name)
            print("----------------正在爬取%s的订阅者第%d页" % (name,page + 1))
            subs_size = 0
            count = 0
            # 因为有时会出现连接失败，返回的页面是空的，所以反复请求连接直到有页面为止
            while (subs_size == 0):
                ...
                print("-----------------解析%s的订阅者html页面%d次" % (name,count))
                ...
                # 这个是为了请求6页以后，微博限制用户访问列表，不再继续爬
                if count > 5:
                    print("-----------------------------解析%s页面第%d页订阅列表页面失败!!!!" % (name, page + 1))
                    break
            print("---------------休眠两秒后继续爬下一页")
            time.sleep(2)
    except Exception as e:
...
def get_fans(weibo_id, name, headers, fans_list_page):
    ...
    try:
        ...
        for page in range(0, fans_list_page):
            ...
            print("---------------------爬取%s的粉丝列表的网址：  %s\n" %(name, fans_url))
            print("---------------------请求连接到%s的粉丝列表页面"%name)
            print("----------------正在爬取%s的粉丝列表第%d页" % (name,page + 1))
            fans_size = 0
            count = 0
            # 因为有时会出现连接失败，返回的页面是空的，所以反复请求连接直到有页面为止
            while (fans_size == 0):
                ...
                print("-----------------------解析%s的粉丝列表html页面%d次" % (name,count))
...
def get_contents_page(weibo_id, name, headers, pagebar, page):
    ...
    try:
        ...
        print("----------------%s共有%s条微博"%(name,weibo_size))
        if int(weibo_size)==0:
            return 0
            ...
            while (weibo_div_size == 0):
                ...
                params = urllib.parse.urlencode(
                    {'ajwvr': 6, 'domain': 100505, "profile_ftype": 1, 'is_all': 1, 'pagebar': pagebar,
                     ...
                     '__rnd': get_timestamp()})  # 调用接口时所用的参数
                cont_url = api_url + "%s" % (params)
                print("---------------请求连接到%s的微博内容页面:%s" % (name,cont_url))
                response=openlink(cont_url,headers)
                html = response.read().decode()  # 对调用接口后传过来的内容进行解码
                cont_html = json.loads(html)['data']
                print("-----------------解析%s的微博文本内容%d次" % (name,count))
                cont_soup = BeautifulSoup(cont_html, 'html.parser')  # 进行解析
...
if __name__ == "__main__":
    ...
    try:
        ...
        if cookie != False:  # 文件存在，cookie是有值的就执行下面的
            ...
            claw_main(names_list[0],headers)
    except Exception as e:
        logging.error(e)
